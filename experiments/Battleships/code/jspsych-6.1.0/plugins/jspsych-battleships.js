	...
	plugin.trial = function(display_element, trial) {
		...
		p.draw = function() {
			...
			if (hits<num_nonzero | p.millis()-last_click_time<trial.end_screen_time) {
				...
				p.textSize(25);
				p.text(`${trial.grid.length**2-click_log.t.length}`,
				...
				if (hits<num_nonzero) {
					...
					if (trial.cheat) {
						// Description
						text=`Remember, your real task is to trick an independent judge that you are an ordinary participant who doesn't know where the ships are.`;
				...
				for (var i=0; i<trial.grid.length; i++) {
				    ...
				    click_log: click_log,
						...
						points: trial.grid.length*trial.grid[0].length-click_log.t.length,
						cheat: trial.cheat
					};
